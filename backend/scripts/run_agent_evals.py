...
from app.evaluations.agent_eval_runner import (
    AgentEvalRunner,
    load_eval_cases,
    write_json_report,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 HR Agent 评估。",
        add_help=False,
        usage="python scripts/run_agent_evals.py [选项]",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        metavar="样例路径",
        default=BACKEND_DIR / "evals" / "agent_cases",
...
async def main() -> int:
    ...
    latest_json_path = output_dir / "agent_eval_latest.json"
    latest_md_path = output_dir / "agent_eval_latest.md"
    ...
    print(f"响应签名稳定性: {report.average_signature_stability:.4f}")
    print(f"平均延迟: {report.average_latency_ms:.2f} ms")
    metrics = report.quality_metrics
    print(f"意图准确率: {_format_metric(metrics.intent_accuracy)}")
    print(f"Artifact 契约通过率: {_format_metric(metrics.artifact_contract_pass_rate)}")
...
def _format_metric(value: float | None) -> str:
    return "未配置" if value is None else f"{value:.2%}"
